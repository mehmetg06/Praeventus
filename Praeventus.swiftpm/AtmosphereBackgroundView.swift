...
struct AtmosphereBackgroundView: View {
    ...
    private var palette: [Color] {
        ...
        if hotSunny {
            return [
                ...
                Color(red: 1.0, green: 0.78, blue: 0.46).opacity(0.92)
            ]
        }
        switch timeOfDay {
        case .dawn:
    ...
    private var lightField: some View {
        ZStack {
            ...
            if hotSunny {
                ...
                LinearGradient(
                    colors: [Color(red: 1.0, green: 0.80, blue: 0.42).opacity(0.18), .clear, Color(red: 1.0, green: 0.62, blue: 0.25).opacity(0.10)],
        ...
        GeometryReader { geometry in
            ZStack {
                Circle()
                    .fill(
                        RadialGradient(
                            colors: [.white.opacity(0.95), Color(red: 1.0, green: 0.90, blue: 0.54).opacity(0.78), Color(red: 1.0, green: 0.70, blue: 0.24).opacity(0.30), .clear],
                            center: .center,
                            startRadius: 0,
                            endRadius: 142
                        )
                    )
                    .frame(width: breathe ? 258 : 238, height: breathe ? 258 : 238)
                    .blur(radius: 18)
                ...
                Ellipse()
                    .fill(Color(red: 1.0, green: 0.76, blue: 0.30).opacity(0.18))
            ...
            Canvas { context, size in
                ...
                if hotSunny {
                    for index in 0..<3 {
                        ...
                        context.fill(Path(roundedRect: rect, cornerRadius: 60), with: .color(Color(red: 1.0, green: 0.78, blue: 0.42).opacity(0.030 - Double(index) * 0.006)))
                    }
                }
            }
        }
    ...
    private var depthOverlay: some View {
        ZStack {
            LinearGradient(
                colors: [.white.opacity(hotSunny ? 0.055 : 0.035), .clear, Color(red: 0.97, green: 0.58, blue: 0.22).opacity(hotSunny ? 0.16 : 0.0), .black.opacity(baseDarkness)],
                startPoint: .top,
                endPoint: .bottom
            )
            RadialGradient(
...
private struct HotSunnyLayer: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [Color(red: 1.0, green: 0.86, blue: 0.48).opacity(0.16), .clear, Color(red: 0.98, green: 0.50, blue: 0.18).opacity(0.10)],
            ...
            TimelineView(.periodic(from: .now, by: 1.0 / 12.0)) { timeline in
                Canvas { context, size in
                    ...
                    for index in 0..<5 {
                        ...
                        context.stroke(path, with: .color(Color(red: 1.0, green: 0.86, blue: 0.54).opacity(0.040 - Double(index) * 0.004)), style: StrokeStyle(lineWidth: 1.0, lineCap: .round, lineJoin: .round))
                    }
                }
            }
            .blur(radius: 1.1)
...
private struct StormPulseLayer: View {
    ...
    var body: some View {
        ZStack {
            Rectangle().fill(.purple.opacity(pulse ? 0.045 : 0.0)).ignoresSafeArea()
            Circle().fill(.indigo.opacity(pulse ? 0.16 : 0.05)).frame(width: 640, height: 640).blur(radius: 120).offset(x: -130, y: -230)
        }
