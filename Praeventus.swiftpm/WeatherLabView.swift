...
struct WeatherLabView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Hızlı Senaryolar") {
                Button("Yaz Günü") { applyPreset(.clear, temp: 34, humidity: 42, pressure: 1018, wind: 10, rain: 4) }
                Button("Cephe Geçişi") { applyPreset(.rain, temp: 22, humidity: 88, pressure: 1004, wind: 28, rain: 78) }
                Button("Fırtına") { applyPreset(.storm, temp: 27, humidity: 91, pressure: 996, wind: 46, rain: 92) }
                Button("Sis") { applyPreset(.fog, temp: 14, humidity: 96, pressure: 1012, wind: 4, rain: 12) }
            }
        }
    ...
    private var temperatureBinding: Binding<Double> {
        Binding(
            ...
            set: { updateWeather(temperature: $0) }
        )
    }
    ...
    private var windBinding: Binding<Double> {
        Binding(
            ...
            set: { updateWeather(windSpeed: $0) }
        )
    }

    private var rainBinding: Binding<Double> {
        Binding(
            get: { weather.rainProbability },
            set: { updateWeather(rainProbability: $0) }
        )
    }
    ...
    private func applyPreset(_ condition: WeatherCondition, temp: Double, humidity: Double, pressure: Double, wind: Double, rain: Double) {
        var next = weather
        next.city = "Mock City"
        next.country = "Weather Lab"
